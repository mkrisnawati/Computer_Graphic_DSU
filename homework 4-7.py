#importing library pygame, numpy, math
import pygame
from sys import exit
import numpy as np
import math

#setting pygame interface
width = 800
height = 600
pygame.init()

screen = pygame.display.set_mode((width, height), 0, 32)
pygame.display.set_caption("Krisnawati Melisa_20245056_Homework 4-7")
...
draw_text("Krisnawati Melisa_20245056_Homework 4-7", font, TEXT_COL, 0, 10)

def clearAndRedraw():
    screen.fill(WHITE)
    #Line and rects
    for i in range(count - 1):
        pygame.draw.line(screen, BLACK, pts[i], pts[i+1], 3)
    for i in range(count):
        pygame.draw.rect(screen, GREEN, (pts[i][0] - margin, pts[i][1] - margin, 2 * margin, 2 * margin), 5)

    #Buttons
    lagrangeButton.draw(screen, (0, 0, 255))
    bezierButton.draw(screen, (0, 0, 255))
    hermiteButton.draw(screen, (0, 0, 255))
    cubicSplineButton.draw(screen, (0, 0, 255))

    #Text
    if buttonCheck == 1:
        curveType = "Implement Langrage Interpolation"
    elif buttonCheck == 2 or buttonCheck == -1:
        curveType = "Implement Cubic Bezier"
    elif buttonCheck == 3:
        curveType = "Implement Cubic Hermite Interpolation"
    else:
        curveType = "Implement Cubic spline"

    font = pygame.font.SysFont("arialblack", 20)
    curveText = font.render(curveType, True, BLACK)
    screen.blit(curveText, (400,5))
...
def bezier():
    ...
    for t in np.arange(0, 1, 0.01):
        ...
        pygame.draw.circle(screen, GOLDEN_BROWN, z.astype(int), 3)
...
def hermiteCubic():
    ...
    for t in np.arange(0, 1, 0.01):
        ...
        for i in np.arange(0, n, 1):
            ...
            pygame.draw.circle(screen, GOLDEN_BROWN, c_h.astype(int), 3)
...
def cubicSpline():
    ...
    for t in np.arange(0, 1, 0.01):
        for i in range(n):
            ...
            pygame.draw.circle(screen, GOLDEN_BROWN, yt.astype(int), 3)
...
def lagrange():
    ...
    for t in np.arange(0, len(pts)-1, 0.01):
        ...
        pygame.draw.circle(screen, GOLDEN_BROWN, ptPlt.astype(int), 3)
...
selectedPoint = -1
lagrangeButton = button((230,230,230),50,500,100,50,"4. Lagrange")
bezierButton = button((230,230,230),200,500,100,50,"5. Bezier")
hermiteButton = button((230,230,230),400,500,100,50,"6. Hermite")
cubicSplineButton = button((230,230,230),600,500,100,50,"7. Cubic Spline")

#pygame.draw.line(screen, (0, 255, 0), (150, 0), (150, 600))
buttonCheck = 1
while not done:
    ...
    if old_pressed == -1 and pressed == 1 and old_button1 == 1 and button1 == 0:
        ...
        if x > 150:
            pts.append(pt)
            count += 1
            pygame.draw.rect(screen, GREEN, (pt[0] - margin, pt[1] - margin, 2 * margin, 2 * margin), 5)
